...
std::string get_new_event_id(const std::set<std::string>& used_events, const std::string& stage_id) {
    auto stage_it = scu::Data::stages.find(stage_id);
    if (stage_it == scu::Data::stages.end()) {
        return ""; // Стадия не найдена
    }

    const scu::Stage& stage = stage_it->second;
    std::vector<std::string> valid_events;

    for (const auto& event_pair : stage.events) {
        const scu::Event& event = event_pair.second;
        ...
        for (const auto& cond : event.conditions_traits_min) {
            auto it = scu::Data::traits.find(cond.first);
            if (it == scu::Data::traits.end() || it->second.value < cond.second) {
                traits_ok = false;
                break;
            }
        }
        ...
        for (const auto& cond : event.conditions_chains) {
            auto it = scu::Data::chains.find(cond.first);
            if (it == scu::Data::chains.end() || it->second != cond.second) {
                chains_ok = false;
                break;
            }
        }
...
int main() {
    ...
    while (!is_died) {
        ...
        //std::string event_id = get_new_event_id(used_events, chains, traits, stages, stage_id);
        std::string event_id = get_new_event_id(used_events, stage_id);

...
std::string get_new_event_id(
	...
	while (!checks_passed) {
		int random = get_random_nmber(0, scu::Data::stages.at(stage_id).events.size());
...
int main() {
	...
	while (!is_died) {
		...
		const scu::Event & event = scu::Data::stages.at(stage_id).events.at(event_id);
		...
		// Check that character still alive. If have ending description game shold end, otherwise game continue.
	...
	for (auto desc : scu::Data::death_descriptions) {
		if (traits.at(desc.trait_id).value >= desc.min_value && traits.at(desc.trait_id).value <= desc.max_value) {
	...
	show_text(result, L"Вы начинаете делать первые шаги!");
}